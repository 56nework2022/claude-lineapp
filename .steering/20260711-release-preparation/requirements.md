...
- [x] applicationId・アプリ表示名が正式なものに変更されている
- [x] 正式なアプリアイコンが全解像度に反映されている
- [x] リリース用keystoreで署名されたAABがビルドできる
- [x] プライバシーポリシーが作成され、公開URLが用意されている（`https://56nework2022.github.io/claude-lineapp/privacy-policy`）
- [x] ストア掲載情報（説明文・スクリーンショット等）の下書きが揃っている（`store-listing-draft.md`）
- [ ] リリースビルドを実機（エミュレータ）にインストールし、フェーズ1機能一式が問題なく動作する → **未達**：devcontainerからWindows側AVDへのadb接続不調のため確認不能。Play Console内部テストトラック経由での物理端末確認に切り替え、ユーザー本人の作業として引き継ぐ（詳細は本ディレクトリの `handoff.md` を参照）
